...
```{r}
data1 <- read.csv("/Users/crlee/R_TEMP/data/rproject/가설1_시각화.csv")
...
predict_dots <- round(ifelse(is.na(data1$일인가구.비율),alone_pred, NA),1)
par(family="AppleGothic")
plot(일인가구.비율 ~ 연도, data= data1) + abline(lm(일인가구.비율 ~ 연도, data= data1),col=c('red')) + plot(일인가구.비율 ~ 연도, data= predict_frame)

predict_frame <- data.frame(연도=data1$연도, 일인가구.비율=predict_dots)
...
# 회귀식의 신뢰구간이 -2만8천에서 -만7천 사이라는 것을 알 수 있다. (회귀식의 기울기가 95%의 확률로 -3만2천-만9천)
...
data3 <- read.csv("/Users/crlee/R_TEMP/data/rproject/가설3_데이터.csv")
data3
```
...
```{r}
data_20s <- read.csv("/Users/crlee/R_TEMP/data/rproject/20대.csv", check.names = FALSE)
...
```{r}
data_30s <- read.csv("/Users/crlee/R_TEMP/data/rproject/30대.csv", check.names = FALSE)
...
# 2010, 2020년 이상치 제거 
data_30s_new <- data_30s[-c(1,11),]
data_30s_new

boxplot(data_30s_new$경제성장률)

# 2020년 이상치 제거 
data_30s_2020 <- data_30s[-11,]
data_30s_2020
boxplot(data_30s_2020$경제성장률)
```
...
```{r}
data_40s <- read.csv("/Users/crlee/R_TEMP/data/rproject/40대.csv", check.names = FALSE)
...
```{r}
data_50s <- read.csv("/Users/crlee/R_TEMP/data/rproject/50대+.csv", check.names = FALSE)
...
##### 설명력이 더 높은 변수
```{r}
mlr <- lm(음주운전수 ~ alone + egr, data = data_50s)
